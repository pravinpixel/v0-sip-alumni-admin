...
class TaskCreatedEmail extends Mailable
{
    ...
    public function __construct($data)
    {
        ...
        Log::channel('cron')->info($this->data);
    ...
    public function build()
    {
        ...
        $auth_user = Employee::where('id', $this->data['created_by'])->with('designation')->first();
        $setting = Setting::where('name', 'signature')->first();
    ...
    private function generateICS($task)
    {
        ...
        $deadline->setTimezone('Asia/Kolkata');
        $deadline->setTime(9, 30);
        $event = Event::create($task['name'])
            ->startsAt($deadline)
            ->endsAt($deadline->copy())
