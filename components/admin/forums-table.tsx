...
const mockPosts = Array.from({ length: 30 }, (_, i) => {
  const reportCount = i % 4 === 0 ? Math.floor(Math.random() * 5) + 1 : 0
  return {
    id: i + 1,
    ...
    reports:
      reportCount > 0
        ? Array.from({ length: reportCount }, (_, j) => ({
            id: j + 1,
            reporterName: `Alumni ${Math.floor(Math.random() * 50) + 1}`,
            reporterProfile: `https://i.pravatar.cc/150?img=${Math.floor(Math.random() * 70) + 1}`,
            center: ["Chennai", "Bangalore", "Mumbai", "Delhi", "Hyderabad", "Pune", "Kolkata"][
            ...
            remarks: `This post contains ${["inappropriate", "offensive", "misleading", "harmful"][Math.floor(Math.random() * 4)]} content that violates community guidelines.`,
            reportedOn: new Date(
              2024,
...
export function ForumsTable() {
  ...
  const [viewingReports, setViewingReports] = useState<(typeof mockPosts)[0] | null>(null)
  ...
  return (
    <Card className="p-6">
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-primary hover:bg-primary">
                ...
                <TableHead className="font-bold text-primary-foreground">Reports</TableHead>
            ...
            <TableBody>
              {filteredPosts
                ...
                .map((post, index) => (
                  <TableRow
                    key={post.id}
                    className={`${index % 2 === 0 ? "bg-background hover:bg-muted/50" : "bg-muted/20 hover:bg-muted/50"} ${post.reportCount > 0 ? "border-l-4 border-l-orange-500" : ""}`}
                  >
                    ...
                    <TableCell>
                      {post.reportCount > 0 ? (
                        <button
                          onClick={() => setViewingReports(post)}
                          className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-orange-100 hover:bg-orange-200 transition-colors"
                        >
                          <AlertTriangle className="h-4 w-4 text-orange-600" />
                          <span className="font-semibold text-orange-700">{post.reportCount}</span>
                        </button>
                      ) : (
      ...
      {viewingReports && <ViewReportsDialog post={viewingReports} onClose={() => setViewingReports(null)} />}
    </Card>
  )
}
