...
<div id="successMessage" class="alert alert-success" style="display: none;">
    <strong>Success:</strong> Location has been successfully updated.
</div>
        ...
        <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Office Location</h1>
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <li class="breadcrumb-item text-muted">
                            <a href="{{url('masters/location')}}" class="text-muted text-hover-primary">Office Location</a>
                        </li>
                    </ul>
                </div>
                <!--end::Page title-->
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                            ...
                            <thead>
                                ...
                                <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                    ...
                                    <th class="min-w-125px">Office Location</th>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body py-10 px-lg-17">
                <form id="ajax-form" method="post" action="{{ isset($location) ? route('location.update', $location->id) : route('location.save') }}">
                    @csrf
...
<script>
    ...
    function loadTableData(searchTerm, selectedStatus, page = '',pageItems='') {
            $.ajax({
                url: "{{ route('location.index') }}?search=" + searchTerm + "&status=" + selectedStatus + "&page=" + page + "&pageItems=" + pageItems,
                type: "GET",
                data: {
                    search: searchTerm,
        ...
        function openModalForAction(action, branchId = null) {
            ...
            var form = $('#ajax-form');
            form.attr('action', action === 'update' ? "{{ route('location.update', ':id') }}" : "{{ route('location.save') }}");
            $('#branchId').val(action === 'update' ? branchId : '');
            // Set the state ID in the hidden input
            form.find('.field-error').text('');

            if (action === 'update') {
                $.get("{{ route('location.get', ':id') }}".replace(':id', branchId), function(data) {
                    $('#name').val(data.name);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (form.attr('action').includes('update')) {
                        ...
                        toastr.success("Location has been successfully updated.");
                    } else {

                        toastr.success("Location has been successfully inserted.");
                    }
                    $('#ajax-form')[0].reset();
        ...
        function refreshTableContent() {
            $.ajax({
                url: "{{ route('location.index') }}", // Replace with the actual route name or URL
                type: "GET",
                dataType: 'html',
                success: function(response) {
