...
<div id="successMessage" class="alert alert-success" style="display: none;">
    <strong>Success:</strong> Task Category has been successfully updated.
</div>
        ...
        <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Task Category</h1>
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <li class="breadcrumb-item text-muted">
                            <a href="{{url('masters/task-category')}}" class="text-muted text-hover-primary">Task Category</a>
                        </li>
                    </ul>
                </div>
                <!--end::Page title-->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body py-10 px-lg-17">
                <form id="ajax-form" method="post" action="{{ isset($task) ? route('task-category.update', $task->id) : route('task-category.save') }}">
                    @csrf
...
<script>
    ...
    $(document).ready(function() {
        ...
        function loadTableData(searchTerm, selectedStatus, page = '' ,pageItems='') {
            $.ajax({
                url: "{{ route('task-category.index') }}?search=" + searchTerm + "&status=" + selectedStatus + "&page=" + page + "&pageItems=" + pageItems,
                type: "GET",
                data: {
                    search: searchTerm,
        ...
        function openModalForAction(action, branchId = null) {
            ...
            var form = $('#ajax-form');
            form.attr('action', action === 'update' ? "{{ route('task-category.update', ':id') }}" : "{{ route('task-category.save') }}");
            $('#branchId').val(action === 'update' ? branchId : '');
            // Set the state ID in the hidden input
            form.find('.field-error').text('');

            if (action === 'update') {
                $.get("{{ route('task-category.get', ':id') }}".replace(':id', branchId), function(data) {
                    $('#name').val(data.name);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (form.attr('action').includes('update')) {
                        ...
                        toastr.success("Task Category has been successfully updated.");
                    } else {

                        toastr.success("Task Category has been successfully inserted.");
                    }
                    $('#ajax-form')[0].reset();
        ...
        function refreshTableContent() {
            $.ajax({
                url: "{{ route('task-category.index') }}", // Replace with the actual route name or URL
                type: "GET",
                dataType: 'html',
                success: function(response) {
