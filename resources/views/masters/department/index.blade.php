...
<div id="successMessage" class="alert alert-success" style="display: none;">
    <strong>Success:</strong> Department has been successfully updated.
</div>
            ...
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <li class="breadcrumb-item text-muted">
                            <a href="{{url('masters/department')}}" class="text-muted text-hover-primary">Department</a>
                        </li>
                    </ul>
                </div>
                <!--end::Page title-->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body py-10 px-lg-17">
                <form id="ajax-form" method="post" action="{{ isset($department) ? route('department.update', $department->id) : route('department.save') }}">
                    @csrf
...
<script>
    ...
    $(document).ready(function() {
        ...
        function loadTableData(searchTerm, selectedStatus, page = '',pageItems='') {
            $.ajax({
                url: "{{ route('department.index') }}?search=" + searchTerm + "&status=" + selectedStatus + "&page=" + page + "&pageItems=" + pageItems,
                type: "GET",
                data: {
                    search: searchTerm,
        ...
        function openModalForAction(action, branchId = null) {
            ...
            var form = $('#ajax-form');
            form.attr('action', action === 'update' ? "{{ route('department.update', ':id') }}" : "{{ route('department.save') }}");
            $('#branchId').val(action === 'update' ? branchId : '');
            // Set the state ID in the hidden input
            form.find('.field-error').text('');

            if (action === 'update') {
                $.get("{{ route('department.get', ':id') }}".replace(':id', branchId), function(data) {
                    $('#name').val(data.name);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (form.attr('action').includes('update')) {
                        ...
                        toastr.success("Department has been successfully inserted.");
                    }
                    $('#ajax-form')[0].reset();
        ...
        function refreshTableContent() {
            $.ajax({
                url: "{{ route('department.index') }}", // Replace with the actual route name or URL
                type: "GET",
                dataType: 'html',
                success: function(response) {
